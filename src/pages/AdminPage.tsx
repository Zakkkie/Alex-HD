...
export const AdminPage: React.FC = () => {
  ...
  // Jellyseerr Docker Integration State
  const [jellyseerrUrlInput, setJellyseerrUrlInput] = useState('http://172.19.0.2:5055');
  const [jellyseerrKeyInput, setJellyseerrKeyInput] = useState('');
  const [jellyseerrEnabled, setJellyseerrEnabled] = useState(true);
  ...
  const [isTestingJellyseerr, setIsTestingJellyseerr] = useState(false);
  const [isSyncingJellyseerr, setIsSyncingJellyseerr] = useState(false);
  const [jellyseerrMsg, setJellyseerrMsg] = useState<{ text: string; type: 'success' | 'error' } | null>(null);
  const [showJellyseerrForm, setShowJellyseerrForm] = useState(false);

  const loadJellyseerrInfo = async () => {
    try {
      const [cfg, status] = await Promise.all([
        api.getJellyseerrConfig().catch(() => null),
        api.getJellyseerrStatus().catch(() => null)
      ]);
      if (cfg) {
        setJellyseerrUrlInput(cfg.url || 'http://172.19.0.2:5055');
        setJellyseerrEnabled(cfg.isEnabled !== false);
      }
      if (status) {
        setJellyseerrStatus(status);
      }
    } catch (e) {
      console.warn('Failed to load Jellyseerr info:', e);
    }
  };

  const handleUpdateJellyseerr = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      if (res.success) {
        ...
        setJellyseerrStatus(status);
        setJellyseerrMsg({
          text: status.online ? 'Подключение к Jellyseerr успешно установлено!' : 'Настройки сохранены. Проверьте сетевую доступность контейнера.',
          type: status.online ? 'success' : 'error'
        });
      }
    } catch (err: any) {
      setJellyseerrMsg({ text: `Ошибка: ${err.message}`, type: 'error' });
    } finally {
      setIsTestingJellyseerr(false);
    }
  };

  const handleSyncJellyseerr = async () => {
    setIsSyncingJellyseerr(true);
    setJellyseerrMsg(null);
    try {
      const res = await api.syncJellyseerrCatalog();
      if (res.success) {
        setJellyseerrMsg({
          text: `Успешно синхронизировано ${res.syncedCount} фильмов/сериалов из Jellyseerr (всего ${res.totalCount})!`,
          type: 'success'
        });
        await loadLocalCatalog();
      } else {
        setJellyseerrMsg({ text: `Синхронизация не удалась: ${res.error || 'Неизвестная ошибка'}`, type: 'error' });
      }
    } catch (err: any) {
      setJellyseerrMsg({ text: `Ошибка синхронизации: ${err.message}`, type: 'error' });
    } finally {
      setIsSyncingJellyseerr(false);
    }
  };
      ...
      {activeTab === 'catalog' && (
        <div className="space-y-6 animate-[fadeIn_0.2s_ease-out]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 font-mono">
            ...
            {/* ========================================================= */}
            {/* JELLYSEERR (OVERSEERR) & DOCKER CLUSTER INTEGRATION CARD */}
            {/* ========================================================= */}
            <div className="p-4 sm:p-5 bg-[#0f0e0d] border border-[#d4b581]/40 rounded-2xl space-y-4 col-span-1 md:col-span-2">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 bg-[#d4b581]/15 border border-[#d4b581]/30 rounded-xl">
                    <Database className="w-5 h-5 text-[#d4b581]" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="text-sm font-bold text-white">Jellyseerr & Docker Media Stack</h4>
                      <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold border ${
                        jellyseerrStatus?.online
                          ? 'bg-emerald-950/60 text-emerald-400 border-emerald-500/40'
                          : 'bg-amber-950/60 text-amber-400 border-amber-500/40'
                      }`}>
                        {jellyseerrStatus?.online ? 'Online (Docker Bridge)' : 'Ожидание связи'}
                      </span>
                    </div>
                    <p className="text-[11px] text-[#e6e3df]/60 mt-0.5">
                      Центральный хаб метаданных: постеры, актёры, режиссёры, описания и запросы в Radarr/Sonarr
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    type="button"
                    onClick={handleSyncJellyseerr}
                    disabled={isSyncingJellyseerr}
                    className="px-3.5 py-1.5 bg-[#d4b581] hover:bg-[#c4a571] text-black font-bold text-[11px] uppercase tracking-wider rounded-lg cursor-pointer disabled:opacity-50 transition-all flex items-center gap-1.5"
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${isSyncingJellyseerr ? 'animate-spin' : ''}`} />
                    <span>{isSyncingJellyseerr ? 'Импорт...' : 'Синхронизировать данные'}</span>
                  </button>

                  <button
                    type="button"
                    onClick={() => setShowJellyseerrForm(!showJellyseerrForm)}
                    className="px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-[#e6e3df]/20 text-white font-bold text-[10px] uppercase tracking-wider rounded-lg cursor-pointer transition-all"
                  >
              ...
              {jellyseerrMsg && (
                <div className={`p-3 rounded-xl border text-xs font-mono flex items-center gap-2 ${
                  ...
                  <span>{jellyseerrMsg.text}</span>
                </div>
              )}

              {showJellyseerrForm && (
                <form onSubmit={handleUpdateJellyseerr} className="space-y-3 pt-3 border-t border-[#e6e3df]/10 animate-[fadeIn_0.2s_ease-out]">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] text-[#e6e3df]/60 font-bold uppercase block mb-1">
                        Адрес Jellyseerr (URL)
                      </label>
                      <input
                        type="text"
                        placeholder="http://172.19.0.2:5055 или http://127.0.0.1:5055"
                        value={jellyseerrUrlInput}
                        onChange={(e) => setJellyseerrUrlInput(e.target.value)}
                        onKeyDown={(e) => e.stopPropagation()}
                        className="w-full px-3 py-1.5 bg-[#171615] border border-[#e6e3df]/20 rounded-lg text-white text-[11px] outline-none font-mono"
                      />
                    </div>
                    ...
                    <div>
                      <label className="text-[10px] text-[#e6e3df]/60 font-bold uppercase block mb-1">
                        API Ключ Jellyseerr (X-Api-Key)
                      </label>
                      <input
                        type="text"
                        placeholder="Вставьте API-ключ из Settings → General в Jellyseerr"
                        value={jellyseerrKeyInput}
                        onChange={(e) => setJellyseerrKeyInput(e.target.value)}
                        onKeyDown={(e) => e.stopPropagation()}
                        className="w-full px-3 py-1.5 bg-[#171615] border border-[#e6e3df]/20 rounded-lg text-white text-[11px] outline-none font-mono"
                      />
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <label className="flex items-center gap-2 text-xs text-white cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={jellyseerrEnabled}
                        onChange={(e) => setJellyseerrEnabled(e.target.checked)}
                        className="rounded border-[#e6e3df]/30 bg-[#171615] text-[#d4b581] focus:ring-0"
                      />
                      <span>Использовать Jellyseerr как приоритетный источник метаданных</span>
                    </label>

                    <button
                      type="submit"
                      disabled={isTestingJellyseerr}
                      className="px-4 py-2 bg-[#d4b581] hover:bg-[#c4a571] text-black font-bold text-[11px] rounded-lg cursor-pointer disabled:opacity-50 transition-all uppercase tracking-wider flex items-center gap-1.5"
                    >
                      <Zap className="w-3.5 h-3.5" />
                      <span>{isTestingJellyseerr ? 'Проверка...' : 'Сохранить и проверить связь'}</span>
                    </button>
                  </div>
                </form>
              )}
