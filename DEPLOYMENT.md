...
1. **Защита доступа администратора**:
   - Все новые посетители и обычные пользователи сайта заходят в роли стандартного пользователя (`Пользователь Alex HD` / `user`).
   - Панель управления узлами и нодами доступна **только владельцу/администратору**.
   - Для входа в режим администратора перейдите в **Аккаунт** -> нажмите кнопку **«Вход для Администратора»** и введите мастер-пароль администратора (`admin123` или `StrongAlexHdPass2026!`).
   - Доступ сохранится в вашей сессии браузера, а при нажатии **«Выйти из админки»** сессия вернется в обычный пользовательский режим.
